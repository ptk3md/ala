...
const CONFIG = {
  ...
  // Coorte adaptativa por estágio (evita comparar 2º período vs 12º)
  COHORT: {
    MIN_SIZE: 25,          // mínimo desejável de alunos comparáveis
    MAX_WINDOW: 3,         // expande até ±3 períodos se necessário
    REQUIRE_REACHED: true, // em métricas de período P, só usa quem já chegou em P
  },
...
function parseDiscColumn(col) {
  const m = col.match(/\((\d+)º Período\)\s+(.+)/);
  if (!m) return null;
  return { period: Number(m[1]), name: String(m[2]).trim() };
}

function buildSchemaFromRow(row) {
  const periodsMap = {};  // period -> [col]
  const discMeta   = {};  // col -> { name, period, category, credits }

  Object.keys(row).forEach(col => {
    const info = parseDiscColumn(col);
    if (!info) return;
    const { period, name } = info;

    (periodsMap[period] ??= []).push(col);
    discMeta[col] = {
      name,
      period,
  ...
  const schemaPeriods = Object.keys(periodsMap).map(Number).sort((a, b) => a - b);
  return { periodsMap, discMeta, schemaPeriods };
}

function getDonePeriodsForStudent(studentRow, periodsMap) {
  const done = new Set();
  Object.keys(periodsMap).map(Number).forEach(p => {
    const cols = periodsMap[p] || [];
    const hasAny = cols.some(col => {
      const g = studentRow[col];
      return (typeof g === 'number') && Number.isFinite(g);
    });
    if (hasAny) done.add(p);
  });
  return done;
}

function getLastCompletedPeriod(studentRow, periodsMap) {
  const done = getDonePeriodsForStudent(studentRow, periodsMap);
  if (!done.size) return 0;
  return Math.max(...done);
}

function calcCRUpToPeriod(studentRow, maxPeriod, periodsMap, discMeta) {
  let sumW = 0, totalW = 0;
  for (let p = 1; p <= maxPeriod; p++) {
    ...
    cols.forEach(col => {
      ...
      const cred = discMeta[col]?.credits ?? 4;
      sumW += g * cred;
      totalW += cred;
    });
  }
  return totalW > 0 ? { cr: sumW / totalW, totalW } : { cr: null, totalW: 0 };
}

function meanTopBottomDecile(arrSortedAsc) {
  const n = arrSortedAsc.length;
  if (!n) return { topMean: null, botMean: null };
  const k = Math.max(1, Math.ceil(n * 0.10));
  const bot = arrSortedAsc.slice(0, k);
  const top = arrSortedAsc.slice(-k);
  return { topMean: mean(top), botMean: mean(bot) };
}

function selectPeerCohort(allUsers, userLastPeriod, periodsMap, discMeta) {
  const usersWithStage = allUsers
    .map(u => ({ u, last: getLastCompletedPeriod(u, periodsMap) }))
    .filter(x => x.last > 0);

  let w = 0;
  let picked = [];
  while (w <= CONFIG.COHORT.MAX_WINDOW) {
    const minP = Math.max(1, userLastPeriod - w);
    const maxP = Math.min(CONFIG.TOTAL_PERIODOS, userLastPeriod + w);
    picked = usersWithStage.filter(x => x.last >= minP && x.last <= maxP);
    if (picked.length >= CONFIG.COHORT.MIN_SIZE) break;
    w++;
  }

  if (picked.length < Math.min(8, CONFIG.COHORT.MIN_SIZE)) picked = usersWithStage;

  const minPicked = picked.length ? Math.min(...picked.map(x => x.last)) : userLastPeriod;
  const maxPicked = picked.length ? Math.max(...picked.map(x => x.last)) : userLastPeriod;

  return {
    peers: picked,               // [{u,last}]
    windowUsed: w,
    range: [minPicked, maxPicked],
    label: `P${minPicked}–P${maxPicked}`,
  };
}

function rankFromValuesDesc(valuesDesc, userValue) {
  let higher = 0;
  for (const v of valuesDesc) if (v > userValue) higher++;
  return higher + 1;
}
...
function processData() {
  ...
  // Schema: colunas (disciplinas) e seus metadados
  const { periodsMap, discMeta, schemaPeriods } = buildSchemaFromRow(user);

  // Períodos efetivamente concluídos pelo aluno (baseado em notas presentes)
  const doneSet     = getDonePeriodsForStudent(user, periodsMap);
  const donePeriods = [...doneSet].sort((a, b) => a - b);
  const userLastPeriod = donePeriods.length ? donePeriods[donePeriods.length - 1] : 0;
  const currentPeriod  = userLastPeriod < CONFIG.TOTAL_PERIODOS ? (userLastPeriod + 1) : userLastPeriod;

  // Coorte comparável (mesmo estágio; janela adaptativa)
  const cohortInfo = selectPeerCohort(allUsers, userLastPeriod || 1, periodsMap, discMeta);

  // Para métricas "atuais", tenta manter só quem já chegou no estágio do aluno
  const cohortNow = cohortInfo.peers
    .filter(x => !CONFIG.COHORT.REQUIRE_REACHED || x.last >= (userLastPeriod || 1));

  const cohortPeers = (cohortNow.length >= 8 ? cohortNow : cohortInfo.peers);
  const cohortUsers = cohortPeers.map(x => x.u);
  const total       = cohortUsers.length;

  // Estruturas auxiliares
  const periodAllMeans   = {};
  const periodUserGrades = {};
  schemaPeriods.forEach(p => { periodAllMeans[p] = []; });

  const catPerPeriod = {};
  schemaPeriods.forEach(p => {
    catPerPeriod[p] = {};
  ...
  // Preenche estatísticas por disciplina / categoria / período dentro da coorte comparável
  cohortPeers.forEach(({ u, last }) => {
    const isUser = String(u['Matrícula']) === userMat;

    schemaPeriods.forEach(p => {
      if (CONFIG.COHORT.REQUIRE_REACHED && last < p) return;
  ...
  // CR do aluno (até o último período concluído)
  const userPack = userLastPeriod ? calcCRUpToPeriod(user, userLastPeriod, periodsMap, discMeta) : { cr: 0, totalW: 0 };
  const userCRWeighted = userPack.cr ?? 0;

  // CRs da coorte (tenta usar o mesmo "horizonte" do aluno; fallback se necessário)
  const cohortCRs = [];
  cohortPeers.forEach(({ u, last }) => {
    const horizon = userLastPeriod ? Math.min(userLastPeriod, last) : last;
    const pack = horizon ? calcCRUpToPeriod(u, horizon, periodsMap, discMeta) : { cr: null, totalW: 0 };
    if (pack.cr !== null) cohortCRs.push(pack.cr);
  });

  // Rank & percentile current (dentro da coorte comparável)
  const sortedCRs = [...cohortCRs].sort((a, b) => b - a);
  const userRank  = sortedCRs.length ? rankFromValuesDesc(sortedCRs, userCRWeighted) : 0;
  const userPct   = sortedCRs.length ? Math.round((userRank / sortedCRs.length) * 100) : 0;

  // Stats por período (apenas períodos concluídos pelo aluno)
  const statsPerPeriod = donePeriods.map(p => {
    const arr = [...periodAllMeans[p]].sort((a, b) => a - b);
    const m   = mean(arr);
    const sd  = stdDev(arr);
    const { topMean, botMean } = meanTopBottomDecile(arr);
    return {
      period:       p,
  ...
  // Ranking ao longo do tempo (acumulado até cada período concluído; dentro da coorte)
  const rankingHistory = donePeriods.map(p => {
    const usersRanks = cohortPeers
      .filter(x => !CONFIG.COHORT.REQUIRE_REACHED || x.last >= p)
      .map(({ u }) => {
        const pack = calcCRUpToPeriod(u, p, periodsMap, discMeta);
        return { mat: String(u['Matrícula']), cr: pack.cr ?? 0 };
      })
      .sort((a, b) => b.cr - a.cr);

    const pos = usersRanks.findIndex(r => r.mat === userMat) + 1;
    const pct = usersRanks.length ? Math.round((pos / usersRanks.length) * 100) : 0;
  ...
  // Trend: média dos últimos 2 períodos concluídos vs CR atual
  ...
  // CR por fase (melhoria 6) — baseado apenas em períodos concluídos
  const crPorFase = CONFIG.FASES.map(fase => {
    const cols = fase.periodos.flatMap(p => periodsMap[p] || []);
    let s = 0, w = 0;
    cols.forEach(col => {
      const g = user[col];
      if (typeof g === 'number' && Number.isFinite(g)) { s += g * discMeta[col].credits; w += discMeta[col].credits; }
    });
  ...
  return {
    ...
    // novos (para timeline/coorte adaptativa)
    donePeriods,
    userLastPeriod,
    currentPeriod,
    cohortInfo,
...
function renderOverviewCharts(data) {
  ...
  const sp = fd.statsPerPeriod
    .filter(s => s.studentMean !== null && s.period <= (data.userLastPeriod ?? Math.max(...fd.periods)))
    .sort((a, b) => a.period - b.period);

  const labels = sp.map(s => `${s.period}º P`);
  const stu  = sp.map(s => +s.studentMean.toFixed(2));
  ...
  makeChart('chart-growth', 'line', {
    labels,
    datasets: [
      { label:'-1 DP', data:loDP, borderColor:'transparent', backgroundColor:'rgba(22,163,74,.1)', fill:false, tension:.4, pointRadius:0 },
...
function renderTimeline(container) {
  ...
  const donePeriods = data.donePeriods;
  ...
  if (roadmap) {
    Array.from({ length: totalPeriods }, (_, i) => i + 1).forEach(p => {
      ...
      const isCurrent = (p === data.currentPeriod);
  ...
  const update = () => {
    ...
    makeChart('chart-sim', 'line', {
      ...
      datasets: [
        ...
        { label:'Projeção', data:allData.map((d,i)=>d.future?d.v:(i===findLastIndexCompat(allData, x => !x.future)?d.v:null)), borderColor:'#d97706', borderDash:[5,4], tension:.4, borderWidth:2, pointBackgroundColor:'#d97706', pointRadius:4, fill:false, spanGaps:true },
